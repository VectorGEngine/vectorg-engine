//! ## URDF loader for the VectorG Engine physics engine
//!
//! VectorG Engine is a set of 2D and 3D physics engines for games, animation, and robotics. The `vectorg-engine-urdf`
//! crate lets you convert an URDF file into a set of rigid-bodies, colliders, and joints, for usage with the
//! `vectorg_engine` physics engine.
//!
...
//!
//! - Mesh file types are limited. Contributions are welcome. You may check the `vectorg-engine-meshloader`
...
#[cfg(doc)]
use vectorg_engine::dynamics::Multibody;
...
/// An urdf link loaded as a VectorG Engine [`RigidBody`] and its [`Collider`]s.
#[derive(Clone, Debug)]
pub struct UrdfLink {
...
/// An urdf joint loaded as a VectorG Engine [`GenericJoint`].
#[derive(Clone, Debug)]
pub struct UrdfJoint {
    /// The VectorG Engine version for the corresponding urdf joint.
...
/// A robot represented as a set of VectorG Engine rigid-bodies, colliders, and joints.
#[derive(Clone, Debug)]
...
/// Handle of a joint read from the URDF file and inserted into VectorG Engine’s `ImpulseJointSet`
...
/// The handles related to a link read from the URDF file and inserted into VectorG Engine’s
...
/// Handles to all the VectorG Engine objects created when inserting this robot into VectorG Engine’s
...
impl UrdfRobot {
    /// Parses a URDF file and returns both the VectorG Engine objects (`UrdfRobot`) and the original urdf
    ...
    /// - `options`: customize the creation of VectorG Engine objects from the URDF description.
    ...
    /// Parses a string in URDF format and returns both the VectorG Engine objects (`UrdfRobot`) and the original urdf
    /// structures (`Robot`). Both structures are arranged the same way, with matching indices for each part.
    ///
    /// If the URDF file references external meshes, they will be loaded automatically if the format
    /// is supported. The format is detected from the file’s extension. All the mesh formats are
    ...
    /// From an already loaded urdf file as a `Robot`, this creates the matching VectorG Engine objects
    ...
    /// Inserts all the robots elements to the VectorG Engine rigid-body, collider, and impulse joint, sets.
    ///
    ...
    /// Inserts all the robots elements to the VectorG Engine rigid-body, collider, and multibody joint, sets.
    ///
...
fn urdf_to_colliders(
    ...
    match &geometry {
        ...
        Geometry::Cylinder { radius, length } => {
            ...
            // instead of VectorG Engine’s default Y-up.
        ...
        Geometry::Mesh { .. } => {
            log::error!("Mesh loading is disabled by default. Enable one of the format features (`stl`, `collada`, `wavefront`) of `vectorg-engine-urdf` for mesh support.");
        }
        ...
        Geometry::Mesh { filename, scale } => {
            ...
            let Ok(loaded_mesh) = vectorg_engine_meshloader::load_from_path(
                full_path,
                &vectorg_engine::prelude::MeshConverter::TriMeshWithFlags(options.trimesh_flags),
                scale,
            ) else {
                return Vec::new();
            };

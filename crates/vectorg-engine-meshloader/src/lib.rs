...
use std::path::Path;
use vectorg_engine::geometry::{MeshConverter, SharedShape};
use vectorg_engine::math::{Isometry, Point, Real, Vector};
use vectorg_engine::prelude::MeshConverterError;
...
pub enum MeshLoaderError {
    /// An error triggered by VectorG Engine’s [`MeshConverter`].
    #[error(transparent)]
